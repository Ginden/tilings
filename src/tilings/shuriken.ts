...
import { apply, intersectsCenteredSquare, mul, rotation, scaling } from '../geometry.js';
...
const MAX_PROTOTILE_REACH = Math.max(
  ...SHURIKEN_PROTOTILES.flatMap((points) => points.map((point) => Math.hypot(point.x, point.y))),
);
...
function stateIntersectsPatch(state: ShurikenState, limit: number): boolean {
  const points = SHURIKEN_PROTOTILES[state.kind]!.map((point) => apply(state.transform, point));
  return intersectsCenteredSquare(points, limit);
}
...
export function generateShuriken(radius: number): Tile[] {
  ...
  const clipLimit = radius + MAX_PROTOTILE_REACH;
  for (let level = 0; level < levels; level++) {
    states = subdivideShuriken(states).filter((state) => stateIntersectsPatch(state, clipLimit));
  }
